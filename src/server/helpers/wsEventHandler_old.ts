...
//         )
//       : console.log(`Player not found with ID '${playerID.slice(0, 8)}...`);
...
//     if (event === EventType.playerData)
//       return this.updatePlayerData(user, data);
...
//       return this.handleReadyForNextRound(user, data);
//   };
...
//   private joinGame = (
//     user: User,
//     data: EventByName<typeof EventType.joinGame>['data'],
//   ): void => {
...
//   private beginGame = (
//     user: User,
//     data: EventByName<typeof EventType.beginGame>['data'],
//   ): void => {
//     const game = this.games.get(data.gameID);
//     let errorMessage;
//     if (game.hasBegun) errorMessage = 'This game has already begun';
//     if (game.host.id !== user.id)
//       errorMessage = 'Only the host can begin the game';
//     if (errorMessage)
//       return user.send({ event: EventType.error, data: errorMessage });
