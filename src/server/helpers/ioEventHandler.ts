...
export class IOEventHandler {
  ...
  public listener = (socket: Socket): void => {
    const cookies = cookie.parse(socket.request.headers.cookie);

    socket.onAny((event: any, ...args: any[]) => {
      console.log(`got ${event}`);
    });
    socket.on('playerData', (arg: any) => {
      console.log('chalk.green(arg):', JSON.stringify(arg));
    });
  ...
  public middleWare: any = (ws: any, req: any): void => {
    const playerID =
  ...
  private createMessageHandler = (user: User) => (msg: any): void => {
  ...
  private createNewUser = (ws: any): User => {
